...
- 同じ語に複数の例文候補がある場合は、`AUTHORING_STANDARD` §0-1 の順（和歌 → 添付資料 → 古典作品の文章 → 生成した学習用作例）で主例文を選ぶ。ただし、順位は採用可能性を確認できた候補にだけ適用する。候補を保持する場合は語レコードの `examples` に `sourceType` を付ける。添付資料が手元にない状態で、その内容や出典を補わない。
