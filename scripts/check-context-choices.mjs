...
const builder = read("static/choice-builder.js");
const choiceSet = builder.match(/function buildChoices\(word, kind, candidates, fallback, deps\) \{(.*?)\n  \}\n/s)?.[1];
assert.ok(choiceSet, "buildChoices must remain available for the context-choice contract");
...
assert.match(source, /isSafePair: isMeaningSafePair/, "mode-vocab.js must pass the shared meaning guard to the choice builder");
