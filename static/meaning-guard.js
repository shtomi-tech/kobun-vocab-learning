...
const KobunMeaningGuard = (() => {
  ...
  const meaningFamilies = [
    ...
    // 「むつかし」（状態）と「むつかる」（動作）は、空欄からは語形を決められない。
    /不快/,
    // 「ふるさと」と「さと」は、どちらも人の住む場所を指し、文脈で切り分けられない。
    /都があった|自分が住んでいた|実家|人里|自宅/,
    // 「さすが（に）」と「さすがなり」は連用形が同形になり、空欄が一意に決まらない。
    /そうはいって/,
    // 「すなはち」「やがて」「とみなり」はどれも「すぐに」と訳せ、空欄で入れ替えが利く。
    // 「いそぐ」の「急ぐ」は動詞で空欄の語形が変わるため、この群には入れない。
    /すぐに|急激|急である/,
    // 「おこたる」の連用形と名詞「おこたり」は同形。
    /なまける|怠慢/,
    ...
    // 「うたて」「うたてあり」「うたてし」と「うし」「こころうし」は、どれも「いやだ」で入れ替わる。
    /いやだ|いやなことに|嘆かわしい/,
    ...
    /噂/,
    // 以下3件は「仮名表記が同一で漢字だけが違う」組。空欄に入る語形が完全に一致するため、
    // 語義は重ならなくても同時に選択肢へ出すと正答が一意に決まらない。
    /こらえる|慕わしく思う/,
    /固まる|手ですくう/,
    /肩を並べるものがない|どちらとも決まらない/,
    // 第47セットも同表記の組が並ぶ。「け」「こと」「しるし」は仮名では区別できない。
    /（〜の）せい|日常。普段/,
    /^①言葉。$|出来事。事柄。事件。/,
    /前兆|際立っている/,
    // 「たより」と「よすが」は、どちらも「頼りとするもの」で入れ替えが利く。
    /頼りになるもの|頼りとするもの/,
    // 第48セットの同表記の組。「つま」は3語とも空欄が同形、「ありし」「ありつる」と
    // 「あり」「ありありて」も空欄の前後だけでは決められない。
    /端緒|着物の端|からみて）夫/,
    /もとの〜|さきほどの〜/,
    /生きている|生きながらえて/,
  ];
